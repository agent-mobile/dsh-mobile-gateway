/**
 * Host-side WebSocket owner for multiplexed Remote logical streams, serving
 * the gateway's `/m/api/remote.mux` upgrades.
 *
 * Vendored from `@deepseek-ai/dsh-api-gateway`'s `stream-server.ts`
 * (v0.1.2-rc.1, MIT) — the package does not export the mux class, and a
 * byte-identical reimplementation of the framing rules would drift. The
 * stream opener differs only in source: the gateway passes the host
 * `typertGateway` service's public `wireStream`, the same carrier the host's
 * own mux uses, so `$events` generations and `session/follow` streams land in
 * the identical host-side registries that answer `$events/result` HTTP calls.
 * @module
 */
